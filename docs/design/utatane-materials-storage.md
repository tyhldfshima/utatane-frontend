...
- ★**状態の正本は [utatane-focus-screens-spec.md](utatane-focus-screens-spec.md) の §3 K「自分の素材」**（6状態）。数え方と対応も同 §3 の 3-1 の表で数える。
  - 状態の一覧をここに二重に持たない（2026-09-20 に正本化。以前はこの節に6状態を書いていた）。
- この文書は、置き場・読み方・再生の決まり（通り道・持ち主・非公開・合言葉）を正本として持つ。
