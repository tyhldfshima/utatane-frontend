// 同じ試験を、手元の保存（メモリ）と Postgres 版（PGlite に移行ファイルを流したもの）の両方で回す
...
// DB の列が uuid なので、試験の id も uuid の形にする
...
const C = uuid('0000000c', 3)
const MISSING = uuid('0000000f', 999)

const backends: { name: string; make: () => Promise<CoreRepository> }[] = [
  { name: '手元の保存（メモリ）', make: async () => new MemoryRepository() },
  { name: 'Postgres 版（PGlite・移行ファイル 0001）', make: async () => new PgRepository((await freshUtataneDb()).client) },
]

describe.each(backends)('CoreService — $name', ({ make }) => {
  let n = 0
  let t = 0
  ...
  describe('公開時の再検証を通らない Version は公開できない', () => {
    ...
    it('素材は出どころの申告が無いと入れられない', async () => {
      ...
      const [back] = await repo.getMaterials([m.id])
      expect(back.embodiedContributionIds).toEqual([c.id])
      expect(back.provenance?.kind).toBe('self_made')
    })
  })
  ...
  describe('受取人の結び（D-2：初めて貢献を作ったとき）', () => {
    it('中央につながれば受取人 id を結ぶ', async () => {
      const calls: string[] = []
      const ben = uuid('beef0000', 1)
      svc = new CoreService({ repo, newId, now, beneficiaries: { ensureBeneficiary: async ({ tyAccountId }) => { calls.push(tyAccountId); return ben } } })
      const d = await svc.createDraft(A, 'x')
      ...
      expect((await repo.findHolderByAccount(A))?.centralBeneficiaryId).toBe(ben)
  ...
  describe('チャンネルと Version Tree', () => {
    ...
    it('元にする貢献が無ければ由来を作らない', async () => {
      const d = await svc.createDraft(A, 'x')
      await expect(svc.createContribution({ tyAccountId: A, versionId: d.id, roleKindId: 'arrangement', derivedFrom: [{ parentId: MISSING, kind: 'based_on' }] })).rejects.toMatchObject({ code: 'parent_contribution_not_found' })
    })
  })
})
